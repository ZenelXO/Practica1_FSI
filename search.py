...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            print("Generados: ", nodes_generated)
            print("Visitados: ", nodes_visited)
            print("Costo Total: ", node.path_cost)
            return node
        if node.state not in closed:
            closed[node.state] = True
            fringe.extend(node.expand(problem))
            nodes_generated+=len(node.expand(problem))
    return None
...
def branch_and_bounds_graph_search(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return graph_search(problem, MiGestor())

#------------------------------#
# **********NUESTRO*********** #
# -----------------------------#
def metodo_subestimacion(problem):
    """Search the deepest nodes in the search tree first. [p 74]"""
    return graph_search(problem, Heuristica(problem))
